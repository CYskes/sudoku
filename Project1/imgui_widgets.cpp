...
bool ImGui::ButtonCy(const char* label, const char* key, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true); //disabling -> giving it a font size below
    //const ImVec2 label_size{ 0.0f, 30 };
...
bool ImGui::Button(const char* label, const char* key, const ImVec2& size_arg)
{
    return ButtonCy(label, key, size_arg, ImGuiButtonFlags_None);
}
